...
FILE *fp0 = fopen(in, "r+b");
 ...
 FILE* huffman_encode_r(const char*in,const char*out_main,const char*out_help){
    ...
    while (fread(&inputChar, sizeof(char), 1,fp0) == 1) {
        ...
        unsigned char useto1[8]={0b01111111,0b10111111,0b11011111,0b11101111,
                                 0b11110111,0b11111011,0b11111101,0b11111110};
        for(int i=0;i<huffman_code[inputChar+CHAR/2].huffmanCode[100];++i) {
            if(count==8) {
                fwrite(&cunchu, sizeof(char), 1, fp1);
                count = 0;
                cunchu=0b11111111;
            }else {
 ...
 FILE* huffman_decode_r(const char*in0,const char*in1,const char*out) {
    ...
    for (int i = 0; i < into; ++i) {
        ...
        if(df == EOF) {
            ...
            fclose(In0);
            fclose(Out);
            fwrite("副输入文件错误", sizeof("副输入文件错误"), 1, stderr);
            return NULL;
    ...
    buildHuffmanTree(&huffmanList);
    Node *root = huffmanList->nodeData; // 假设 huffmanList->nodeData 是根节点
    int as[100]; int top = 0;
    FILE* df= fopen("D:\\testhrtjrtn.txt", "w+b");
    printCodes(root, as, top, df);
    unsigned char bit;
    Node *currentNode = root;
    int use[8];
    unsigned char useto1[8]={0b01111111,0b10111111,0b11011111,0b11101111,
                                 0b11110111,0b11111011,0b11111101,0b11111110};
    while (fread(&bit,1,1,In0) == 1) {
        for(int i=0;i<8;++i) {
            use[i] = bit !=( bit & useto1[i]);
        }
        for(int i=0;i<8;++i) {
            if(use[i]==0) {
                currentNode = currentNode->leftChild;
