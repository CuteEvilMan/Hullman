...
inline int compareFrequencies(ListNode *nodeA, ListNode *nodeB);
...
FILE* huffman_encode(const char*,const char*);
FILE* huffman_decode(const char*,const char*);
...
inline FILE *huffman_decode(const char *in, const char *out) {
FILE *fp0 = fopen(in, "r+b");
        ...
        /*int codeArray[100], top = 0;
        huffman_code=(HuffmanCode*)malloc(CHAR*sizeof(HuffmanCode));
        printCodes(root, codeArray, top, fp1);
